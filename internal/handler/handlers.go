...
func (h *Handler) V1Encrypt(c *gin.Context) {
	...
	if _, err := ssh.ParsePrivateKey(secret); err == nil {
		...
		c.IndentedJSON(http.StatusOK, v1.RestEncryptResult{
...
func (h *Handler) V1CheckPaas(c *gin.Context) {
	var input v1.RestCheckPaasInput
	if err := c.BindJSON(&input); err != nil {
		c.IndentedJSON(http.StatusBadRequest, v1.RestCheckPaasResult{Error: err.Error()})
		return
	}
	...
	if err != nil {
		if strings.Contains(err.Error(), "unable to decrypt data") ||
			...
			c.IndentedJSON(http.StatusUnprocessableEntity, v1.RestCheckPaasResult{
	...
	c.IndentedJSON(http.StatusOK, v1.RestCheckPaasResult{
